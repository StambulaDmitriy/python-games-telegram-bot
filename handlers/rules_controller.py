...
def get_games_rules():
    return {
        ...
        'blackjack': "Правила игры в Блекджек: Цель игры добирать в руку карты и набрать больше очков, чем диллер, но не более 21. Короли, Дамы и Валеты стоят 10 очков. Тузы оцениваются по ситуации: Если в руку помещается 11 очков - туз оценивается в 11 очков, если не помещается - в 1 очко. Карты с 2 до 10 стоят своего номинала.\nВ случае победы ставка Х1.5, при ничьей ставка возвращается",
    }


async def rules_command(message: types.Message):
    await message.answer('Выбери игру по которой необходима информация:', reply_markup=keyboards.inline.rules_inline_keyboard)


async def rules_inline_callback(call: types.CallbackQuery, callback_data: RulesCallbackData):
    await call.message.edit_text(get_games_rules()[callback_data.game], reply_markup=keyboards.inline.rules_inline_keyboard)
    await call.answer()
